...
public:
    ...
    void UpdateParameters(float size, float dense, float speed, float index) 
    {
        damping_     = (0.9f * dense) + 0.0008f;
        bandwidth_   = 1.0f - damping_;
        decay_       = (0.8f * size) + 0.1f;
        decay_atten_ = 1.0f - powf(decay_, 2.0f);

        float osc_freq = (9.0f * speed) + 1.0f;
        float osc_amp  = (96.0f * index) + 4.0f;
        ...
        // Die Signale per Referenz (&) ausgeben mit echtem Überkreuz-Feedback (Crosstalk)
    inline void Process(float in_l, float in_r, float out_gain, float &out_wet_l, float &out_wet_r) 
    {
